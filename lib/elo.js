...
async function findEloPlacement(newGame, existingEloGames, user, state = {}) {
  console.log('[DEBUG] findEloPlacement called');
console.log('  newGame:', newGame?.game);
const otherGames = user.gameElo.filter(e => String(e.game) !== String(newGame.game));
console.log('  otherGames.length:', otherGames?.length);
  ...
  if (prevMid !== undefined && winner) {
    ...
    if (opponent) {
      ...
      if (isPillar) {
        ...
        console.log('[DEBUG] Updating ratings: newGame', newGame.elo, 'vs', opponent.elo, 'winner', winner);

      }
      const newGameIdStr = String(extractGameId(newGame.game));
      const userEntry = user.gameElo.find(e => String(extractGameId(e.game)) === String(newGame.game));      
      if (userEntry) {
        if (!userEntry.comparisonHistory) userEntry.comparisonHistory = [];
      
        userEntry.comparisonHistory.push({
          againstGame: oppId,
          preferred: winner === 'new',
          timestamp: new Date()
        });
        console.log('[DEBUG] Added to comparisonHistory:', userEntry?.comparisonHistory?.slice(-1));

      
        user.markModified('gameElo'); // <-- VERY IMPORTANT
      }
  ...
  if (left > right) {
    ...
    user.markModified('gameElo');
    await user.save();
    console.log('[DEBUG] Returning result from findEloPlacement:', result);
  ...
  // Return partial progress so controller can persist current elo
  return {
    elo: newGame.elo,
    finalized: false,
